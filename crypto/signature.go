...
func (s *Secp256k1Signature) Parity() uint {
	// odd  V => parity = 0
	// even V => parity = 1
	return s.V.Bit(0) ^ 0x01
}

func (s *Secp256k1Signature) BigParity() *big.Int {
	return new(big.Int).SetInt64(int64(s.Parity()))
}
...
func (s *EIP155Signature) ToCompactSignature() ([]byte, error) {
	v := new(big.Int)
	v.Add(s.BigParity(), btcecRecoveryIDOffset)
...
func (sig *Signature) GetEthSignature(chainID string) (*EIP155Signature, error) {
	...
	return &EIP155Signature{
		Secp256k1Signature{
			V: *GetEthSignatureRecoveryID(chainID, compactSig.BigParity()),
			R: compactSig.R,
			S: compactSig.S,
		},
	}, nil
